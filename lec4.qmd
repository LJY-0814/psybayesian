...
```{r}
#| include: false
knitr::opts_chunk$set(fig.path = "assets/figs/lec4/")
...
pacman::p_load(tidyverse,gridExtra,papaja,here)
...
|---------------|-------------------------------|------------------------|--------------------------------|------------------------------|  
| **单个事件**   | 袋子里四个小球中有几个是蓝色       |  均匀的可能性             |  画出各种可能组合的全部路径     |  根据观测数据“消除”不可能路径         |  
| **离散变量**   | 多次试验(多次进行重复实验)的成功率  | 指定的三种成功率(0.2, 0.5, 0.8)及其可能性  | 进行重复后的结果在**三种**成功率下出现的可能性 | 简单的手动计算 |  
| **连续变量**   | 多次试验(多次进行重复实验)的正确率  | 符合正确率(0~1)数据特点和先验经验的概率分布  | 50个试次中正确30次在**所有可能**正确率下出现的可能性 | 已被证明的统计学公式|
...
```{r}
# here::here() 以项目根目录为锚点，避免依赖当前工作目录
...
我们的先验分布为$Beta(70, 30)$，那么编号为“82111”的被试在253个试次中，有152次判断为正确的可能性（这节课中我们假设正确率为 $\theta$ ），这个似然函数可以用**二项分布**来表示：  
...
与上节课相同，我们的正确率 $\theta$ 服从$Beta$ 分布，似然函数为**二项分布**：  
...
- **结论**  
  ...
  2. 样本量越大，似然对正确率$\theta$的约束力越强，后验分布也会更加集中于观测结果，这使得我们对$\theta$的推断更为确定。  
...
- **先验信念的影响逐渐减弱**：最初的先验分布提供了对正确率 $\theta$ 的初步信念，但随着更多数据被观察到，先验对后验的影响逐渐减弱。  
...
<center><img src='figs/lec4/skex9x185z_2x.gif' style='height:360px;'></center>  
...
<center><img src='figs/lec4/fig1_lrg.jpg' style='height:480px;'></center>  

> Shooshtari, et al (2019). *Neuroscience*, doi:10.1016/j.neuroscience.2019.03.031  
> Evans, et al., (2020). *Journal of Experimental Psychology: Learning, Memory, and Cognition*, doi: 10.1037/xlm0000725 
...
1. 第一种先验和上节课相同，认为参数值$\theta$(正确率) 在 0.7 左右，用 Beta(70,30)表示。  
   
2. 第二种先验比较极端，认为参数值$\theta$(正确率)大于 0.6，并且其值越大约有可能，用 Beta(10,1)表示。  
   
3. 最后一种“躺平”的思路认为，参数在$\theta$ 0-1 之间出现的可能性是完全相同的，即先验可以用均匀分布表示 Beta(1,1)。
...
* 在上图中，不同的先验，反映了研究者对正确率的不同信念(认为$\theta$主要集中分布在哪里)  
...
1. **模糊型先验下**：  
   - 如果**研究者对正确率没有明确预期**，后验分布几乎完全由数据（似然分布）决定。由于模糊型先验（如 $\text{Beta}(1, 1)$）不提供关于 $\theta$ 的有效信息，后验与似然分布几乎重合。  

2. **信息型先验下**：  
   - 当**研究者对正确率有较强的信念**，后验分布依然会受到先验的显著影响。例如，使用 $\text{Beta}(10, 1)$ 作为信息型先验，后验分布会集中在 0.6 - 0.8 区域，反映了研究者对 $\theta$ 的较高信心。  
...
- 例如，研究者可能对随机点运动任务的正确率持有固执的观点，坚信其正确率较低。他们可能认为 $\theta$ 的任何值在 0 到 0.25 之间都是等可能的，并坚决认为其不会超过 0.25。为了表达这种先验信念，可以采用 0 到 0.25 之间的均匀模型（uniform model）。  
...
<center><img src='figs/lec4/ch4-stubborn-plot-1.png' style='height:420px;'></center>
...
这个数学结果的含义是巨大的。  
- 无论实验者收集到多少相反的证据，他的后验概率永远不会超过 0.25 的上限 （“ **** 限制了我的想象力”）
...
Tips：如何避免令人遗憾的先验模型  
	- 确保对每个可能的$\theta$值都分配非零的可信度，即使这个可信度接近于零。例如，如果$\theta$是一个可以从 0 到 1 的比例，那么你的先验模型也应该在这个连续范围内进行定义。  

![](figs/lec2/rhqcb9gji7.png){height=360} 
...
下面列出了五种不同的先验模型。  
请用以下描述之一标记每个先验：有些偏向 $\theta$ < 0.5，有些强烈偏向$\theta$ < 0.5，有些将$\theta$置于0.5中心，有些偏向$\theta$ > 0.5，有些强烈偏向$\theta$ > 0.5。  
- a) Beta(1.8,1.8)  
- b) Beta(3,2)  
- c) Beta(1,10)  
- d) Beta(1,3)  
...
![](figs/lec4/unnamed-chunk-142-1.png){width=48%}  
